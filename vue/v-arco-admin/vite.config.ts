...
export default defineConfig(({ mode }: ConfigEnv): UserConfig => {
  ...
  const { VITE_PORT, VITE_PUBLIC_PATH, VITE_API_PROXY_URL, VITE_APP_TITLE, VITE_APP_SHORT_TITLE } = env
  // const isBuild = command === 'build'
  const isHttps = /^https:\/\//.test(VITE_API_PROXY_URL)
  const isProd = /prod|stg|stage/i.test(mode)
    ...
    server: {
      ...
      proxy: {
        '/api': {
          ...
          /* fix 访问其他环境api时会报403|404，去掉origin重启服务 */
          headers:{
            origin:''
          },
        }
